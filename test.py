...
def test(opt):
    ...
    with torch.no_grad():
        if opt.benchmark_all_eval:  # evaluation with 10 benchmark evaluation datasets
            ...
            if opt.by_length:
                _, accuracy, norm_ED, preds_str, confidence_score_list, _, _, length_of_data = validation_by_length(
                    model, criterion, evaluation_loader, converter, opt)
                log.write(eval_data_log)
                length_key = sorted(accuracy.keys())
                for length in length_key:
                    print(f'Length: {length}, ACC: {accuracy[length]:0.3f}, norm_ED: {norm_ED[length]:0.3f}')
                    log.write(f'Length: {length}, ACC: {accuracy[length]:0.3f}, norm_ED: {norm_ED[length]:0.3f}\n')
                log.close()
            else:
